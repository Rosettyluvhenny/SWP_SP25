    ...
    const handleEditFeedback = async (feedback: Feedback) => {
        ...
        try {
            if (feedback.serviceId) {
                ...
                const serviceDetails = await serviceDataById(
                    feedback.serviceId.toString()
                );

                if (serviceDetails) {
                    // Create a booking object from service data and feedback
                    const serviceBooking: Booking = {
                        id: feedback.id?.toString() || "",
                        serviceId: feedback.serviceId,
                        serviceName: feedback.serviceName,
                        img: feedback.img || serviceDetails.img,
                        status: "COMPLETED",
                        paymentStatus: "PAID",
                        paymentMethod: "",
                        notes: "",
                        sessionRemain: serviceDetails.session || 0,
            ...
            toast.error("Đã xảy ra lỗi khi tải thông tin dịch vụ.");
        }
    };
            ...
            if (isEditing && currentFeedbackId) {
                ...
                if (success) {
                    ...
                    setCompletedFeedbacks((prev) =>
                        prev.map((f) =>
                            f.id?.toString() === currentFeedbackId
                                ? {
                                      ...f,
                                      feedbackText,
                                      rating,
