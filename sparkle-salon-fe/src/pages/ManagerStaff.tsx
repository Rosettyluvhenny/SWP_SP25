...
export default function ManagerStaff() {
  ...
  const [checkTimes, setCheckTimes] = useState<{
    [key: string]: { checkIn: string | null; checkOut: string | null };
  }>({});
  ...
  const handleCheckIn = (orderId: string) => {
    setCheckTimes((prev) => ({
      ...prev,
      [orderId]: prev[orderId]?.checkIn
        ? prev[orderId]
        : {
            checkIn: getCurrentTime(),
            checkOut: prev[orderId]?.checkOut || null,
          },
    }));
  };
  ...
  const handleCheckOut = (orderId: string) => {
    setCheckTimes((prev) => ({
      ...prev,
      [orderId]: prev[orderId]?.checkOut
        ? prev[orderId]
        : {
            checkIn: prev[orderId]?.checkIn || null,
  ...
  const handleSelectRoom = (orderId: string, room: string) => {
    setSelectedRoom((prev) => ({ ...prev, [orderId]: room }));
  };
            ...
            <tbody className="bg-white">
              {orders.map((order) => (
                <tr key={order.id} className="border-t">
                  ...
                  <td className="p-3">
                    {checkTimes[order.id]?.checkIn || "Chưa Check-in"}
                  </td>
                  <td className="p-3">
                    {checkTimes[order.id]?.checkOut || "Chưa Check-out"}
                  </td>
                  <td className="p-3">
                    <select
                      onChange={(e) =>
                        handleSelectStaff(order.id, e.target.value)
                      }
                  ...
                  <td className="p-3">
                    <select
                      onChange={(e) =>
                        handleSelectRoom(order.id, e.target.value)
                      }
                      value={selectedRoom[order.id] || ""} // ✅ Fixed state
                      className="border p-2 rounded"
                    >
                  ...
                  <td className="p-3 flex space-x-2">
                    <button
                      ...
                      disabled={!!checkTimes[order.id]?.checkIn}
                    >
                      Check-in
                    </button>
                    <button
                      ...
                      disabled={!!checkTimes[order.id]?.checkOut}
                    >
                      Check-out
                    </button>
                  </td>
